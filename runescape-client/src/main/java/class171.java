			...
			if (Client.titleLoadingStage == 50) {
				...
				if (Client.titleLoadingStage == 70) {
					if (!ObjectSound.archive2.isFullyLoaded()) {
						...
						FillMode.setObjectDefinitionArchives(ObjectSound.archive2, class40.archive7, Client.isLowDetail);
					...
					if (GrandExchangeOfferUnitPriceComparator.compass == null) {
						GrandExchangeOfferUnitPriceComparator.compass = class322.SpriteBuffer_tryCreateSprite(WorldMapSection2.archive8, Calendar.spriteIds.compass, 0);
					} else {
						++var0;
					}

					if (FaceNormal.redHintArrowSprite == null) {
						FaceNormal.redHintArrowSprite = class322.SpriteBuffer_tryCreateSprite(WorldMapSection2.archive8, Calendar.spriteIds.redHintArrow, 0);
					} else {
						++var0;
					}
					...
					if (class192.mapSceneSprites == null) {
						...
						if (!SpriteMask.SpriteBuffer_loadSprite(var30, var34, 0)) {
							var32 = null;
						} else {
							var32 = WorldMapLabel.SpriteBuffer_createIndexedSpriteArray();
						}
					...
					if (WorldMapData_0.headIconPkSprites == null) {
						...
						if (!SpriteMask.SpriteBuffer_loadSprite(var30, var34, 0)) {
							...
							var35 = UserComparator9.SpriteBuffer_createSpriteArray();
						}
