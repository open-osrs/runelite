...
public class FontName {
	...
	static int method5639(int var0, Script var1, boolean var2) {
		if (var0 == ScriptOpcodes.FRIEND_COUNT) {
			if (WorldMapArea.friendSystem.field357 == 0) {
				...
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = WorldMapArea.friendSystem.friendsList.getSize();
			}
		...
		if (var0 == ScriptOpcodes.FRIEND_GETNAME) {
			...
			if (WorldMapArea.friendSystem.method99() && var3 >= 0 && var3 < WorldMapArea.friendSystem.friendsList.getSize()) {
		...
		if (var0 == ScriptOpcodes.FRIEND_TEST) {
			var4 = Interpreter.Interpreter_stringStack[--Interpreter.Interpreter_stringStackSize];
			var4 = NPCDefinition.removeImageTag(var4);
		...
		if (var0 == ScriptOpcodes.CLAN_GETCHATCOUNT) {
			if (PacketWriter.clanChat != null) {
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = PacketWriter.clanChat.getSize();
			} else {
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = 0;
			}
		...
		if (var0 == ScriptOpcodes.CLAN_GETCHATUSERNAME) {
			var3 = Interpreter.Interpreter_intStack[--RouteStrategy.Interpreter_intStackSize];
			if (PacketWriter.clanChat != null && var3 < PacketWriter.clanChat.getSize()) {
		...
		if (var0 == ScriptOpcodes.IGNORE_COUNT) {
			if (!WorldMapArea.friendSystem.method99()) {
				...
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = WorldMapArea.friendSystem.ignoreList.getSize();
			}
		...
		if (var0 == ScriptOpcodes.IGNORE_GETNAME) {
			var3 = Interpreter.Interpreter_intStack[--RouteStrategy.Interpreter_intStackSize];
			if (WorldMapArea.friendSystem.method99() && var3 >= 0 && var3 < WorldMapArea.friendSystem.ignoreList.getSize()) {
		...
		if (var0 == ScriptOpcodes.CLAN_ISSELF) {
			var3 = Interpreter.Interpreter_intStack[--RouteStrategy.Interpreter_intStackSize];
			if (PacketWriter.clanChat != null && var3 < PacketWriter.clanChat.getSize() && PacketWriter.clanChat.get(var3).getUsername().equals(Canvas.localPlayer.username)) {
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = 1;
			} else {
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = 0;
			}
		...
		if (var0 == ScriptOpcodes.CLAN_ISFRIEND) {
			var3 = Interpreter.Interpreter_intStack[--RouteStrategy.Interpreter_intStackSize];
			if (PacketWriter.clanChat != null && var3 < PacketWriter.clanChat.getSize() && ((ClanMate)PacketWriter.clanChat.get(var3)).isFriend()) {
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = 1;
			} else {
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = 0;
			}
		...
		if (var0 == ScriptOpcodes.CLAN_ISIGNORE) {
			var3 = Interpreter.Interpreter_intStack[--RouteStrategy.Interpreter_intStackSize];
			if (PacketWriter.clanChat != null && var3 < PacketWriter.clanChat.getSize() && ((ClanMate)PacketWriter.clanChat.get(var3)).isIgnored()) {
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = 1;
			} else {
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = 0;
			}
	...
	@Export("clearCaches")
	static final void clearCaches() {
		class4.FloorOverlayDefinition_clearCached();
		Tile.FloorUnderlayDefinition_clearCached();
		DevicePcmPlayerProvider.KitDefinition_clearCached();
		WorldMapManager.ObjectDefinition_clearCached();
		WorldMapLabel.NpcDefinition_clearCached();
		LoginScreenAnimation.ItemDefinition_clearCached();
		SequenceDefinition.SequenceDefinition_cached.clear();
		SequenceDefinition.SequenceDefinition_cachedFrames.clear();
		UserComparator3.SpotAnimationDefinition_clearCached();
		VarbitDefinition.VarbitDefinition_cached.clear();
		VarpDefinition.VarpDefinition_cached.clear();
		GrandExchangeEvents.HitSplatDefinition_clearCached();
		Buffer.HealthBarDefinition_clearCached();
		StructDefinition.StructDefinition_cached.clear();
		InvDefinition.ParamDefinition_clearCached();
		Projectile.WorldMapElement_clearCached();
		PlayerAppearance.PlayerAppearance_cachedModels.clear();
		BuddyRankComparator.Widget_clearCached();
