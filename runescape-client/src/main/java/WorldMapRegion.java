...
public class WorldMapRegion {
	...
	static final void checkIfMinimapClicked(Widget var0, int x, int y) {
		...
		int var4 = MouseHandler.MouseHandler_lastPressedX - x;
		int var5 = MouseHandler.MouseHandler_lastPressedY - y;
	...
	@Export("resizeWidget")
	static void resizeWidget(Widget[] components, int var1, int width, int height, boolean runScripts) {
		for (int var5 = 0; var5 < components.length; ++var5) {
			Widget var6 = components[var5];
			if (var6 == null || var6.parentId != var1) {
				continue;
			}

			WorldMapSection2.alignWidgetSize(var6, width, height, runScripts);
			FloorOverlayDefinition.alignWidgetPosition(var6, width, height);
			...
			if (var6.type == 0) {
				WorldMapData_1.revalidateWidgetScroll(components, var6, runScripts);
			}
		}
