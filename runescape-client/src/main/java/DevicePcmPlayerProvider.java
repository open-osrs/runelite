...
public class DevicePcmPlayerProvider implements PcmPlayerProvider {
	...
	@Export("KitDefinition_clearCached")
	public static void KitDefinition_clearCached() {
	...
	static void SpriteBuffer_decode(byte[] data) {
		Buffer var1 = new Buffer(data);
		var1.offset = data.length - 2;
		SpriteBuffer.SpriteBuffer_spriteCount = var1.readUnsignedShort();
		SpriteBuffer.SpriteBuffer_xOffsets = new int[SpriteBuffer.SpriteBuffer_spriteCount];
		SpriteBuffer.SpriteBuffer_yOffsets = new int[SpriteBuffer.SpriteBuffer_spriteCount];
		VarbitDefinition.SpriteBuffer_spriteWidths = new int[SpriteBuffer.SpriteBuffer_spriteCount];
		SecureRandomCallable.SpriteBuffer_spriteHeights = new int[SpriteBuffer.SpriteBuffer_spriteCount];
		SpriteBuffer.SpriteBuffer_pixels = new byte[SpriteBuffer.SpriteBuffer_spriteCount][];
		var1.offset = data.length - 7 - SpriteBuffer.SpriteBuffer_spriteCount * 8;
		...
		for (var3 = 0; var3 < SpriteBuffer.SpriteBuffer_spriteCount; ++var3) {
			SpriteBuffer.SpriteBuffer_xOffsets[var3] = var1.readUnsignedShort();
		}
		...
		for (var3 = 0; var3 < SpriteBuffer.SpriteBuffer_spriteCount; ++var3) {
			VarbitDefinition.SpriteBuffer_spriteWidths[var3] = var1.readUnsignedShort();
		}
		...
		var1.offset = data.length - 7 - SpriteBuffer.SpriteBuffer_spriteCount * 8 - (var2 - 1) * 3;
		SpriteBuffer.SpriteBuffer_spritePalette = new int[var2];

		for (var3 = 1; var3 < var2; ++var3) {
			SpriteBuffer.SpriteBuffer_spritePalette[var3] = var1.readMedium();
			if (SpriteBuffer.SpriteBuffer_spritePalette[var3] == 0) {
				SpriteBuffer.SpriteBuffer_spritePalette[var3] = 1;
			}
		}
		...
		for (var3 = 0; var3 < SpriteBuffer.SpriteBuffer_spriteCount; ++var3) {
			int var4 = VarbitDefinition.SpriteBuffer_spriteWidths[var3];
			int var5 = SecureRandomCallable.SpriteBuffer_spriteHeights[var3];
			int var6 = var4 * var5;
	...
	static final void getActorScreenLocation(Actor actor, int height) {
		WorldMapLabel.worldToScreen(actor.x, actor.y, height);
	}
