...
public class WorldMapID {
	...
	static int method573(int var0, Script var1, boolean var2) {
		...
		if (var0 == ScriptOpcodes.STRUCT_PARAM) {
			...
			var8 = class229.getParamDefinition(var7);
			if (var8.isString()) {
				Interpreter.Interpreter_stringStack[++Interpreter.Interpreter_stringStackSize - 1] = GZipDecompressor.StructDefinition_getStructDefinition(var6).getStringParam(var7, var8.defaultStr);
			} else {
				Interpreter.Interpreter_intStack[++RouteStrategy.Interpreter_intStackSize - 1] = GZipDecompressor.StructDefinition_getStructDefinition(var6).getIntParam(var7, var8.defaultInt);
			}
