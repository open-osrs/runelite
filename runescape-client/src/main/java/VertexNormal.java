...
public class VertexNormal {
	...
	static synchronized void method2984(byte[] var0) {
		...
		} else if (RouteStrategy.ByteArrayPool_arrays != null) {
			for (int var1 = 0; var1 < ByteArrayPool.ByteArrayPool_alternativeSizes.length; ++var1) {
				if (var0.length == ByteArrayPool.ByteArrayPool_alternativeSizes[var1] && WorldMapSection1.ByteArrayPool_altSizeArrayCounts[var1] < RouteStrategy.ByteArrayPool_arrays[var1].length) {
					RouteStrategy.ByteArrayPool_arrays[var1][WorldMapSection1.ByteArrayPool_altSizeArrayCounts[var1]++] = var0;
					return;
				}
			}
		}
