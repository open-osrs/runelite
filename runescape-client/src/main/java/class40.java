...
public class class40 {
	...
	static final void drawEntities(int var0, int var1, int var2, int var3) {
		...
		class229.checkLocalPlayerDestination();
		WorldMapDecoration.addLocalPlayerToScene();
		TriBool.addTargetPlayerToScene();
		BufferedSource.addNpcsToScene(true);
		...
		BufferedSource.addNpcsToScene(false);
		...
		for (Projectile var7 = (Projectile)Client.projectiles.last(); var7 != null; var7 = (Projectile)Client.projectiles.previous()) {
			if (var7.plane == SoundSystem.plane && Client.cycle <= var7.cycleEnd) {
				if (Client.cycle >= var7.cycleStart) {
					...
					class65.scene.drawEntity(SoundSystem.plane, (int)var7.x, (int)var7.y, (int)var7.z, 60, var7, var7.yaw, -1L, false);
				}
			} else {
				var7.remove();
			}
			...
			if (var21.plane == SoundSystem.plane && !var21.isFinished) {
				if (Client.cycle >= var21.cycleStart) {
					...
					if (var21.isFinished) {
						...
						class65.scene.drawEntity(var21.plane, var21.x, var21.y, var21.height, 60, var21, 0, -1L, false);
					}
				}
			} else {
				var21.remove();
		...
		GrandExchangeOfferUnitPriceComparator.drawHintArrow(var0, var1);
