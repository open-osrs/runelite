...
public class GargsPrayerOverlay
        extends Overlay
{
    ...
    @Inject
    private GargsPrayerOverlay(Client client, GargsPlugin plugin, SpriteManager spriteManager)
    {
    ...
    public Dimension render(Graphics2D graphics)
    {
        ...
        if ((plugin.isInGargs()) && (plugin.getPrayAgainst() != null) && (plugin.getDusk() != null))
        {
            ...
            LocalPoint duskPoint = new LocalPoint(dusk.getLocalLocation().getX() + 128 * (dusk.getTransformedComposition().getSize() - 1) / 2, dusk.getLocalLocation().getY() + 128 * (dusk.getTransformedComposition().getSize() - 1) / 2);
